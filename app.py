...
if st.sidebar.button("🔄 Clear Cached Data RAM"):
    st.cache_data.clear()
...
def get_metric_colors(metric_name):
    ...
    if "actual" in m_lower:
        return "#0072B2", "#56B4E9"
    elif "lasso" in m_lower:
        return "#D55E00", "#FFB591"
    elif "sp" in m_lower or "scenario_qty_sp" in m_lower:
        return "#009E73", "#89E5C9"
    elif "wmape" in m_lower:
        return "#CC79A7", "#F0D0E0"
    else:
        return "#F0E442", "#FFF79A"
...
if analysis_mode == "Single Report Mode":
    uploaded_file = st.file_uploader(
        "📁 Upload Excel Report File", type=["xlsx", "xls"]
    )
    ...
    if uploaded_file:
        ...
        if "_model" in selected_sheet.lower():
            ...
            for col in filter_targets:
                ...
                selected_vals = st.sidebar.multiselect(
                    ...
                    key=f"s_mod_val_{col}",
                    placeholder=f"All {col} values active",
                )
                if selected_vals:
            ...
            selected_metrics = st.multiselect(
                "📊 Select Metrics to View on Plot",
                numeric_cols,
            ...
            if not filtered_df.empty and selected_metrics:
                ...
                agg_df = (
                    filtered_df.groupby(date_col)[selected_metrics]
                    .sum()
                    .reset_index()
                ...
                for metric in selected_metrics:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="lines+markers",
                            name=metric,
                            line=dict(color=curr_color, width=3),
                            marker=dict(size=6),
                        )
                    )

                fig.update_layout(
                    title=f"Trend Analysis Over Time ({date_col})",
                    xaxis_title=date_col,
                    yaxis_title="Sum of Quantities",
                    hovermode="x unified",
                    plot_bgcolor="white",
                ...
                st.warning(
                    "⚠️ No data available to draw timeline plot based on active selections."
                )
        ...
        # --- CASE A2: METRIC SHEETS ---
            ...
            metric_view_layout = st.radio(
                ...
                options=[
                    "Pivot Table Matrix Explorer",
                    "Raw Table Cell Viewer (e.g., WMAPEs)",
                ],
                horizontal=True,
                key="s_metric_layout",
            )
            ...
            if metric_view_layout == "Pivot Table Matrix Explorer":
                ...
                with p_col2:
                    remaining_cat = [
                        c for c in categorical_cols if c != row_sel
                    ] or categorical_cols
                    bin_idx = next(
                        (
                            i
                ...
                with p_col3:
                    val_idx = next(
                        (
                            ...
                            if "itm" in c.lower() or "id" in c.lower()
                        ),
                        len(all_cols) - 1,
                    )
                    val_sel = st.selectbox(
                        "Values to Aggregate", all_cols, index=val_idx, key="s_val"
                    )
                with p_col4:
                ...
                try:
                    if filtered_df.empty:
                        ...
                        st.dataframe(
                            pivot_matrix.style.background_gradient(
                ...
                default_keys = [
                    c
                    for c in all_cols
                    if any(x in c.lower() for x in ["rfm", "itm", "id", "menu"])
                    and not any(m in c.lower() for m in ["wmape", "actual", "qty"])
                ]
                ...
                if not id_cols:
                    ...
                    st.dataframe(
                        df_display.style.format(format_rules), use_container_width=True
                    )
...
# ==========================================
    ...
    with col1:
        uploaded_old = st.file_uploader(
            "📁 Upload PREVIOUS Month Excel (Old)", type=["xlsx", "xls"]
        )
    with col2:
        uploaded_new = st.file_uploader(
            "📁 Upload CURRENT Month Excel (New)", type=["xlsx", "xls"]
        )
    ...
    if uploaded_old and uploaded_new:
        ...
        if not common_sheets:
            ...
            selected_sheet = st.sidebar.selectbox(
                "🎯 Select Sheet to Analyze", common_sheets
            )
            st.subheader(f"Comparing Sheet: `{selected_sheet}`")

            df_old_raw, df_new_raw = load_sheet_comparison(
                uploaded_old, uploaded_new, selected_sheet
            )
            ...
            if "_model" in selected_sheet.lower():
                date_col = next(
                    (
                        ...
                        if "dt" in c.lower()
                        or "date" in c.lower()
                        or pd.api.types.is_datetime64_any_dtype(df_new[c])
                    ),
                    all_cols[0],
                )
                numeric_cols = [
                    c
                    for c in all_cols
                    if pd.api.types.is_numeric_dtype(df_new[c]) and c != date_col
                ]
                ...
                selected_metrics = st.multiselect(
                    "📊 Select Metrics to Compare on Plot",
                    numeric_cols,
                    default=(
                        numeric_cols[:3] if len(numeric_cols) >= 3 else numeric_cols
                    ),
                )

                combined_df = pd.concat(
                    [filtered_df_old, filtered_df_new], ignore_index=True
                )
                ...
                if not combined_df.empty and selected_metrics:
                    ...
                    agg_df = (
                        combined_df.groupby(groupby_cols)[selected_metrics]
                        .sum()
                        .reset_index()
                        .sort_values(by=date_col)
                    )
                    ...
                    for metric in selected_metrics:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=old_data[metric],
                                mode="lines+markers",
                                name=f"⏮️ {metric} (Prev Month)",
                                line=dict(color=prev_color, dash="dot", width=2),
                                marker=dict(size=5),
                            )
                        )
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=new_data[metric],
                                mode="lines+markers",
                                name=f"🆕 {metric} (Curr Month)",
                                line=dict(color=curr_color, width=3.5),
                                marker=dict(size=7),
                            )
                        )

                    fig.update_layout(
                        title=f"Comparison over Time ({date_col})",
                        xaxis_title=date_col,
                        yaxis_title="Sum of Quantities",
                        hovermode="x unified",
                        plot_bgcolor="white",
            ...
            # --- CASE B2: METRIC SHEETS ---
                ...
                metric_view_layout = st.radio(
                    ...
                    options=[
                        "Pivot Table Matrix Explorer",
                        "Unified Interleaved Table Cell Comparison",
                    ],
                    horizontal=True,
                    key="c_metric_layout",
                )
                ...
                if metric_view_layout == "Pivot Table Matrix Explorer":
                    ...
                    with p_col3:
                        ...
                        val_sel = st.selectbox(
                            "Values to Aggregate", all_cols, index=val_idx, key="c_val"
                        )
                    with p_col4:
                        agg_func = st.selectbox(
                            "Aggregation Function",
                    ...
                    try:
                        if filtered_df_old.empty or filtered_df_new.empty:
                            ...
                            pivot_old_aligned, pivot_new_aligned = pivot_old.align(
                                pivot_new, fill_value=0
                            )
                            delta_pivot = pivot_new_aligned - pivot_old_aligned

                            st.markdown("---")
                            st.markdown(
                                "### 🗺️ Side-by-Side Monthly Distribution Matrix"
                            )
                            m_col1, m_col2 = st.columns(2)
                            with m_col1:
                                st.subheader("⏮️ Previous Month Pivot")
                                st.dataframe(
                                    pivot_old.style.background_gradient(
                                        cmap="Blues", axis=None
                                    ),
                                    use_container_width=True,
                                )
                            with m_col2:
                                ...
                                st.dataframe(
                                    pivot_new.style.background_gradient(
                            ...
                            st.markdown("---")
                            st.markdown(
                                "### 🔄 Bin Shifting & Delta Analysis (Current - Previous)"
                            )
                            st.dataframe(
                                delta_pivot.style.format("{:+}").background_gradient(
                ...
                # --- SUB-ROUTE 2: UNIFIED INTERLEAVED COMPONENT ---
                else:
                    default_keys = [
                        c
                        for c in all_cols
                    ...
                    if not id_cols:
                        ...
                        if not selected_cols:
                            ...
                            for metric in selected_cols:
                                ...
                                unified_df[c_old] = pd.to_numeric(
                                    (
                                        raw_old_indexed[metric]
                                        if metric in raw_old_indexed.columns
                                        else pd.NA
                                    ),
                                    errors="coerce",
                                )
                                unified_df[c_new] = pd.to_numeric(
                                    (
                                        raw_new_indexed[metric]
                                        if metric in raw_new_indexed.columns
                                        else pd.NA
                                    ),
                                    errors="coerce",
                                )
                                unified_df[c_delta] = (
                                    unified_df[c_new] - unified_df[c_old]
                                )

                                interleaved_columns_list.extend([c_old, c_new, c_delta])

                                if (
                                    "wmape" in metric.lower()
                                    or "error" in metric.lower()
                                ):
                                    formatting_rules_map[c_old] = lambda x: (
                                        f"{x*100:.2f}%" if pd.notnull(x) else "-"
                                    )
                                    formatting_rules_map[c_new] = lambda x: (
                                        f"{x*100:.2f}%" if pd.notnull(x) else "-"
                                    )
                                    formatting_rules_map[c_delta] = lambda x: (
                                        f"{x*100:+.2f}%" if pd.notnull(x) else "0.00%"
                                    )
                                    ...
                                    formatting_rules_map[c_delta] = lambda x: (
                                        f"{x:+,.2f}" if pd.notnull(x) else "0.00"
                                    )

                            final_render_df = unified_df[
                                interleaved_columns_list
                            ].sort_index()
                            st.markdown(
                                "### 📋 Unified Interleaved Grain Comparison View"
                            )

                            styled_matrix = final_render_df.style.format(
                                formatting_rules_map
                            ).background_gradient(
                                cmap="RdYlGn_r", subset=delta_wmape_cols, axis=0
                            )
                            st.dataframe(styled_matrix, use_container_width=True)
else:
    st.info(
        "👋 Upload source files via the widget to initialize the data inspection suite."
    )
